...
### Fixed
- `getReferenceDateAndTime` no longer truncates the ephemeris time to whole seconds. All four sensor models cast it to `time_t` before formatting, so images that fall in the same second got an identical reference time, and thus an identical serial number when ISIS builds one from it. jigsaw then rejected the framelets of a framing instrument such as JunoCam as duplicate serial numbers. The time is now kept as a `double` so the sub-second part survives. [#534](https://github.com/DOI-USGS/usgscsm/pull/534)
