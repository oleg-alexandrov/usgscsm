...
- Detector sample and line summing (`m_detectorSampleSumming`, `m_detectorLineSumming`) are read from the model state as `double` again, not `int`. These fields are declared `double`, written with `set<double>`, and are genuinely fractional for some sensor models, but the read was narrowed to `get<int>` in [#507](https://github.com/DOI-USGS/usgscsm/pull/507). That truncated a fractional summing to `0`, producing a divide-by-zero (an `inf` sample) in `groundToImage` for linescan and frame models that use fractional summing, such as the SPOT, Pléiades, and PeruSat cameras built by Ames Stereo Pipeline. Integer-summing sensors were unaffected. [#523](https://github.com/DOI-USGS/usgscsm/pull/523)
