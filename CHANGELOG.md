...
### Changed
- When constructing a sensor model from an ISD, the distortion type is now resolved with this library's own `getDistortionModel(json)` reading the ISD distortion name directly, instead of routing the name through ale's `getDistortionModel` and its integer enum. usgscsm now uses its own `DistortionType` enum consistently in every code path rather than sometimes deferring to ale's. After the ale enum was aligned to usgscsm's in ale [#728](https://github.com/DOI-USGS/ale/pull/728) the two are nominally identical, so this is a consistency change with no effect on any recognized model. As part of this, `getDistortionModel(json)` now throws on an unrecognized or missing distortion model rather than silently defaulting to `TRANSVERSE`, matching ale's behavior. The `legendre` (New Horizons) and `themisir` (THEMIS IR) names that ale can emit remain unimplemented here and now throw an error, exactly as they already did through the ale path; adding support for those sensors is tracked in [#527](https://github.com/DOI-USGS/usgscsm/issues/527). Done with AI (Claude) assistance. [#526](https://github.com/DOI-USGS/usgscsm/pull/526)
